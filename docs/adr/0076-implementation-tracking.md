# ADR 0076 Implementation Tracking

ADR: [0076-ontogpt-inspired-native-linkml-schema-governance-and-recursive-spires-extraction.md](0076-ontogpt-inspired-native-linkml-schema-governance-and-recursive-spires-extraction.md)

## Manager Rules

- Scope is core schema-guided recursive extraction bundle contract only: no LinkML parser, no SPIRES prompt engine, no JSON-LD/RDF/OWL support, no graph/Kuzu access, no file-system access in core, no sidecar, no LLM calls, no MCP tools, no graph/audit schema changes, no dependency additions.
...
- Source search found existing graph schema manifest, ontology validation report, hypothesis grounding, contract versions, and sidecar enrichment surfaces.
- Source search found no existing `ontoindex/src/core/extraction/schema-guided-extraction.ts`, `ExtractionBundle`, `SchemaGuidedExtraction`, or `buildSchemaGuidedExtractionReport`.
...
| --- | --- | --- | --- | --- |
| T1 core module and focused tests | sub-agent T1 | done | `ontoindex/src/core/extraction/schema-guided-extraction.ts`, `ontoindex/test/unit/schema-guided-extraction.test.ts` | `npm test -- --run test/unit/schema-guided-extraction.test.ts`; `npx tsc --noEmit --pretty false` |
...
- 2026-06-10: T2 completed with focused unit tests, TypeScript check, and scoped diff check passing; manager integration closeout started.
- 2026-06-10: Integrated validation passed: focused schema-guided extraction unit suite, TypeScript check, scoped `git diff --check`, forbidden dependency search, and local OntoIndex CLI status.
- 2026-06-10: ADR 0076 status and ADR index updated to `Implemented (core extraction contract)`.
