 ...
 *
 * Historically defined in `ontoindex/src/core/ingestion/model/symbol-table.ts`;
 * moved into `ontoindex-shared` as part of RFC #909 Ring 1 (#910) so the
 * scope-resolution types that reference it can live in the shared package
 * alongside their consumers (`ontoindex/` and `ontoindex-web/`).
 *
