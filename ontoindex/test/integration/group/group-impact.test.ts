...
beforeAll(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'ontoindex-grp-impact-int-'));
...
describe('group impact integration', () => {
  ...
  it('runs happy-path stub against fixture group (stops before bridge when no symbol UIDs)', async () => {
    const prev = process.env.ONTOINDEX_HOME;
    process.env.ONTOINDEX_HOME = tmpHome;
    try {
      const svc = new GroupService(stubPort());
      ...
      if (prev === undefined) delete process.env.ONTOINDEX_HOME;
      else process.env.ONTOINDEX_HOME = prev;
    }
  });
});
