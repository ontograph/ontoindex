...
describe('buildImportedReturnTypeMaps', () => {
  it('builds simple and raw return maps with one symbol lookup per import', () => {
    const namedImportMap = new Map([
      [
        'src/app.ts',
        new Map([
          ['loadUsers', { sourcePath: 'src/users.ts', exportedName: 'loadUsers' }],
          ['loadConfig', { sourcePath: 'src/config.ts', exportedName: 'loadConfig' }],
          ['missing', { sourcePath: 'src/missing.ts', exportedName: 'missing' }],
        ]),
      ],
    ]);
    const lookupExactFull = vi.fn((filePath: string, name: string) => {
      if (filePath === 'src/users.ts' && name === 'loadUsers') {
        return { returnType: 'Promise<User>' };
      }
      if (filePath === 'src/config.ts' && name === 'loadConfig') {
        return { returnType: 'Config' };
      }
      return undefined;
    });

    const { importedReturnTypes, importedRawReturnTypes } = buildImportedReturnTypeMaps(
      'src/app.ts',
      namedImportMap,
      { lookupExactFull },
    );

    expect(lookupExactFull).toHaveBeenCalledTimes(3);
    expect(importedReturnTypes.get('loadUsers')).toBe('User');
    expect(importedReturnTypes.get('loadConfig')).toBe('Config');
    expect(importedReturnTypes.has('missing')).toBe(false);
    expect(importedRawReturnTypes.get('loadUsers')).toBe('Promise<User>');
    expect(importedRawReturnTypes.get('loadConfig')).toBe('Config');
    expect(importedRawReturnTypes.has('missing')).toBe(false);
  });
});
