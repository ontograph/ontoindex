...
describe('impact: batching and grouping', () => {
  ...
  it('batches 250 IDs into 3 chunked STEP_IN_PROCESS queries', async () => {
    ...
    const repoHandle = {
      ...
      lbugPath: '/tmp/repo/.ontoindex/lbug',
      indexedAt: 'now',
      lastCommit: 'c',
      stats: {},
    } as any;
    ...
    const res = await (backend as any).impact(repoHandle, params);
  ...
  it('groups entry points across chunks and deduplicates correctly', async () => {
    ...
    const repoHandle = {
      ...
      name: 'repo2',
      repoPath: '/tmp/repo2',
      storagePath: '/tmp/repo2/.ontoindex',
      lbugPath: '/tmp/repo2/.ontoindex/lbug',
      indexedAt: 'now',
      lastCommit: 'c',
      stats: {},
    } as any;
  ...
  it('caps enrichment to MAX_CHUNKS and sets partial when capped', async () => {
    ...
    const repoHandle = {
      ...
      name: 'repo3',
      repoPath: '/tmp/repo3',
      storagePath: '/tmp/repo3/.ontoindex',
      lbugPath: '/tmp/repo3/.ontoindex/lbug',
      indexedAt: 'now',
      lastCommit: 'c',
      stats: {},
    } as any;
