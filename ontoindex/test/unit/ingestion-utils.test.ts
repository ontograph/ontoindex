...
describe('findDescendant', () => {
  const makeNode = (type: string, children: SyntaxNode[] = []): SyntaxNode =>
    ({
      type,
      children,
    }) as unknown as SyntaxNode;

  it('walks very deep ASTs without using the JS call stack', () => {
    let leaf = makeNode('target');
    for (let i = 0; i < 20_000; i++) {
      leaf = makeNode(`wrapper_${i}`, [leaf]);
    }

    expect(findDescendant(leaf, 'target')?.type).toBe('target');
  });

  it('returns null when the requested descendant is absent', () => {
    expect(findDescendant(makeNode('root', [makeNode('child')]), 'missing')).toBeNull();
  });
});
