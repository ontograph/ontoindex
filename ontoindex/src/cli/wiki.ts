 ...
 * Usage: ontoindex wiki [path] [options]
 */
...
export const wikiCommand = async (inputPath?: string, options?: WikiCommandOptions) => {
  console.warn(
    '\n  DEPRECATION WARNING: `ontoindex wiki` is deprecated.\n' +
      '  Use `ontoindex audit [--annotate]` for structured audit reports.\n' +
      '  For architecture exploration:\n' +
      '    READ ontoindex://repo/{name}/clusters\n' +
  ...
  if (!meta) {
    console.log('  Error: No OntoIndex index found.');
    console.log('  Run `ontoindex analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (
    ...
    console.log('  Config saved to ~/.ontoindex/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      ...
      if (!llmConfig.apiKey && llmConfig.provider !== 'cursor') {
        ...
        console.log('  Set OPENAI_API_KEY or ONTOINDEX_API_KEY environment variable,');
      ...
      if (choice === '5' && hasCursor) {
        ...
        const envKey = process.env.ONTOINDEX_API_KEY || process.env.OPENAI_API_KEY || '';
        let azureKey: string;
        if (envKey) {
  ...
  const wikiOptions: WikiOptions = {
    ...
    maxTokenBudget: options?.maxTokens ? parseInt(options.maxTokens, 10) : undefined,
  };
  ...
  try {
    ...
    if (options?.review && result.moduleTree) {
      ...
      if (choice === 'n' || choice === 'no') {
        console.log('\n  Generation cancelled. Run `ontoindex wiki` later to generate.\n');
        return;
      }
      ...
      if (choice === 'edit' || choice === 'e') {
        ...
        try {
          ...
          console.log('  Then run `ontoindex wiki` to continue.\n');
          return;
        }
      }
    ...
    if (message?.includes('No source files')) {
      console.log(`\n  ${message}\n`);
    } else if (message?.includes('content filter')) {
    ...
    } else if (message?.includes('API key') || message?.includes('API error')) {
      console.log(`\n  LLM Error: ${message}\n`);
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `ontoindex wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${message}\n`);
      if (process.env.ONTOINDEX_VERBOSE) {
        console.error(err);
      }
    }
    process.exitCode = 1;
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync(
      'gh',
      ['gist', 'create', htmlPath, '--desc', 'Repository Wiki — generated by OntoIndex', '--public'],
      {
        encoding: 'utf-8',
