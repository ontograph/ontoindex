...
- Prefer high-fidelity live diagrams using the special tag: <!-- gn-mermaid: [type] query -->
  - type: 'call_flow' — use for dependency graphs between functions
  - type: 'inter_module' — use for module relationship graphs
...
- Include a high-level Mermaid architecture diagram showing only the most important modules and their relationships (max 10 nodes). Prefer the live tag: <!-- gn-mermaid: inter_module [query] -->
...
export function formatFileListForGrouping(
  ...
  return files
    .map((f) => {
      const sortedSymbols = [...f.symbols].sort(
