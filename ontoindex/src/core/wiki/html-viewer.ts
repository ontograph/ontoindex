...
const MAX_EMBEDDED_WIKI_BYTES = (() => {
  const raw = Number.parseInt(process.env.ONTOINDEX_MAX_EMBEDDED_WIKI_BYTES ?? '', 10);
  return Number.isFinite(raw) ? Math.max(0, raw) : 10 * 1024 * 1024;
})();
...
export async function generateHTMLViewer(wikiDir: string, projectName: string): Promise<string> {
  ...
  for (const f of dirEntries) {
    ...
    if (embeddedBytes > MAX_EMBEDDED_WIKI_BYTES) {
      throw new Error(
        `Wiki HTML viewer would embed ${embeddedBytes} bytes of markdown; cap is ${MAX_EMBEDDED_WIKI_BYTES}. ` +
          'Use the markdown files directly or raise ONTOINDEX_MAX_EMBEDDED_WIKI_BYTES.',
      );
    }
  }
  for (const f of dirEntries) {
...
function buildHTML(
  ...
  parts.push('<div class="sidebar-footer">Generated by OntoIndex</div>');
  parts.push('</nav>');
