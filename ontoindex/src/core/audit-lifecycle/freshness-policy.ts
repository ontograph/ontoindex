...
export interface AuditFreshnessPolicyInput {
  ...
  graphAuthorityState?: 'authoritative' | 'review' | 'degraded' | null;
}
...
export function evaluateFreshnessGatePolicy(
  ...
  const graphAuthoritative = input.graphAuthorityState === 'authoritative';
  const dirtyWorktree = input.dirtyWorktree === true && !graphAuthoritative;
  const authorityFailure =
    input.graphAuthorityState === 'review' || input.graphAuthorityState === 'degraded';
  const graphStale =
    explicitStale || authorityFailure || (indexedHead !== null && indexedHead !== targetHead);
  const sourceFresh = !explicitStale && !sourceHeadMismatch && !dirtyWorktree && !authorityFailure;
...
function freshnessReasonCodes(input: {
  ...
  if (input.authorityFailure) reasonCodes.add('graph-authority');
  return [...reasonCodes];
}
