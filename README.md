...
| **GitNexus** | Predecessor codebase and donor project for local code graph indexing | Local repository graph and MCP-oriented code intelligence concepts | Earlier MCP/code-navigation surfaces that informed OntoIndex | OntoIndex is the hard-cutover successor: renamed package/CLI/config, AGPL-licensed release artifacts, expanded MCP frontier, stronger repo-scope safeguards, docs/audit workflows, and current public distribution under `ontograph/ontoindex` |
| [Graphify](https://github.com/safishamsi/graphify) | Review-oriented project graph artifacts and change-analysis reports | Python/NetworkX-style artifact graph with report-friendly graph exports | Not primarily an OntoIndex-style persistent MCP index; useful as a reference for graph-aware review artifacts | OntoIndex keeps the graph local and queryable through MCP/CLI, adds freshness/provenance checks, and implements Graphify-inspired review diagnostics inside existing diff, docs, and audit surfaces rather than adopting an artifact-first runtime |
